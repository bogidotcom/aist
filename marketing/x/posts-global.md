...
**English** — `img/en/01.png`

```
AIST P2P is live.

One book: BTC, USDT on five networks, KGST, ETH, SOL — and fiat. Bank transfer, card, mobile wallet, cash.
...
```
AIST P2P ya está en vivo.

Un solo libro: BTC, USDT en cinco redes, KGST, ETH, SOL — y fiat. Transferencia bancaria, tarjeta, billetera móvil, efectivo.
...
```
O AIST P2P está no ar.

Um único livro: BTC, USDT em cinco redes, KGST, ETH, SOL — e fiat. Transferência bancária, cartão, carteira móvel, dinheiro.
...
```
منصة AIST P2P أصبحت متاحة.

دفتر واحد: BTC وUSDT على خمس شبكات وKGST وETH وSOL — والعملات الورقية. حوالة بنكية، بطاقة، محفظة موبايل، نقداً.
...
```
‏AIST P2P راه‌اندازی شد.

یک دفتر واحد: BTC، USDT روی پنج شبکه، KGST، ETH، SOL — و فیات. حواله بانکی، کارت، کیف پول موبایل، نقدی.
...
```
‏AIST P2P اب لائیو ہے۔

ایک ہی بُک: BTC، پانچ نیٹ ورکس پر USDT، KGST، ETH، SOL — اور فیاٹ۔ بینک ٹرانسفر، کارڈ، موبائل والٹ، نقد۔
...
একটাই বুক: BTC, পাঁচটি নেটওয়ার্কে USDT, KGST, ETH, SOL — আর ফিয়াট। ব্যাংক ট্রান্সফার, কার্ড, মোবাইল ওয়ালেট, নগদ।
...
**English** — `img/en/02.png`

```
We wrote the guide we wanted when we started.
...
```
Escribimos la guía que nos hubiera servido al empezar.
...
```
Escrevemos o guia que queríamos ter quando começamos.
...
```
كتبنا الدليل الذي تمنيناه عندما بدأنا.
...
```
راهنمایی را نوشتیم که آرزو داشتیم موقع شروع داشته باشیم.
...
```
وہ گائیڈ لکھی جو شروع میں ہمیں خود درکار تھی۔
...
```
শুরুর সময় যে গাইডটা চেয়েছিলাম, সেটাই লিখলাম।
...
**English** — `img/en/03.png`

```
Your GPU takes AI jobs and gets paid in compute coins.
...
```
Tu GPU procesa trabajos de IA y cobra en compute coins.
...
```
Sua GPU processa tarefas de IA e recebe em compute coins.
...
```
كرت الشاشة لديك ينفّذ مهام ذكاء اصطناعي ويُدفع له بعملات الحوسبة.
...
```
کارت گرافیک شما کارهای هوش مصنوعی را انجام می‌دهد و با کامپیوت‌کوین پول می‌گیرد.
...
```
آپ کا GPU اے آئی کے کام کرتا ہے اور کمپیوٹ کوائنز میں معاوضہ لیتا ہے۔
...
```
আপনার GPU এআই কাজ করে আর কম্পিউট কয়েনে পেমেন্ট পায়।
...
**English** — `img/en/04.png`

```
The same pair, the same minute, two makers who disagree on price.
...
```
El mismo par, el mismo minuto, dos makers que no coinciden en el precio.
...
```
O mesmo par, o mesmo minuto, dois makers que discordam no preço.
...
```
نفس الزوج، نفس الدقيقة، صانعان لا يتفقان على السعر.
...
```
همان جفت، همان دقیقه، دو میکر که سر قیمت توافق ندارند.
...
```
ایک ہی جوڑا، ایک ہی منٹ، اور دو میکر جو قیمت پر متفق نہیں۔
...
```
একই পেয়ার, একই মিনিট, দুই মেকার যারা দামে একমত নয়।
...
**English** — `img/en/05.png`

```
The same USDT is worth different money depending on how the money moves.
...
```
El mismo USDT vale distinto según cómo se mueve el dinero.
...
```
O mesmo USDT vale valores diferentes conforme o dinheiro se move.
...
```
نفس الـUSDT تختلف قيمته حسب طريقة تحرك المال.
...
```
همان USDT بسته به نحوه حرکت پول ارزش متفاوتی دارد.
...
```
وہی USDT اِس بات پر مختلف قیمت رکھتا ہے کہ پیسہ کیسے حرکت کرتا ہے۔
...
```
একই USDT-র দাম বদলায় টাকা কীভাবে নড়ে তার উপর।
...
**English** — `img/en/06.png`

```
KGST trades on Binance and on our book at the same time.

Two venues, two order books, one asset — and no rule that says they must agree.
...
```
KGST cotiza en Binance y en nuestro libro al mismo tiempo.

Dos plazas, dos libros, un activo — y ninguna regla que los obligue a coincidir.
...
```
KGST negocia na Binance e no nosso livro ao mesmo tempo.

Duas praças, dois livros, um ativo — e nenhuma regra dizendo que precisam concordar.
...
```
‏KGST يُتداول على Binance وعلى دفترنا في نفس الوقت.

منصتان، دفتران، أصل واحد — ولا قاعدة تُلزمهما بالاتفاق.
...
```
‏KGST همزمان در بایننس و در دفتر ما معامله می‌شود.

دو بازار، دو دفتر، یک دارایی — و هیچ قانونی که آن‌ها را به توافق مجبور کند.
...
```
‏KGST بائنانس اور ہماری بُک، دونوں پر ایک ساتھ ٹریڈ ہوتا ہے۔

دو منڈیاں، دو بُکس، ایک اثاثہ — اور کوئی اصول نہیں کہ دونوں کی قیمت ملے۔
...
```
KGST একই সময়ে Binance আর আমাদের বুক—দুই জায়গাতেই ট্রেড হয়।

দুই বাজার, দুই বুক, এক অ্যাসেট — আর মিলতেই হবে এমন কোনো নিয়ম নেই।
...
**English** — `img/en/07.png`

```
Stablecoins cost more where the local currency is under pressure.
...
```
Las stablecoins cuestan más donde la moneda local está bajo presión.
...
```
Stablecoins custam mais onde a moeda local está sob pressão.
...
```
العملات المستقرة أغلى حيث تكون العملة المحلية تحت ضغط.
...
```
استیبل‌کوین‌ها هرجا پول ملی تحت فشار است گران‌ترند.
...
```
جہاں مقامی کرنسی دباؤ میں ہو، وہاں سٹیبل کوائن مہنگا ہوتا ہے۔
...
```
যেখানে স্থানীয় মুদ্রা চাপে, সেখানে স্টেবলকয়েন দামি।
...
**English** — `img/en/08.png`

```
Stop hunting for a spread and quote one instead.
...
```
Deja de buscar el spread y cotízalo tú.
...
```
Pare de caçar o spread e cote você mesmo.
...
```
توقّف عن مطاردة الفارق وسعّره أنت.
...
```
شکار اسپرد را کنار بگذارید و خودتان قیمت بدهید.
...
```
سپریڈ ڈھونڈنا چھوڑیں اور خود قیمت لگائیں۔
...
```
স্প্রেড খোঁজা ছাড়ুন, নিজেই দাম দিন।
...
**English** — `img/en/09.png`

```
Holding a payout for four days instead of selling it Monday is a position, whether you call it one or not.
...
```
Aguantar un cobro cuatro días en vez de venderlo el lunes es una posición, la llames como la llames.
...
```
Segurar um pagamento por quatro dias em vez de vender na segunda é uma posição, chame do que quiser.
...
```
الاحتفاظ بعائدك أربعة أيام بدل بيعه الاثنين هو مركز مفتوح، سمّه ما شئت.
...
```
نگه‌داشتن درآمد به مدت چهار روز به‌جای فروش در دوشنبه، یک پوزیشن است، هر اسمی که رویش بگذارید.
...
```
پیر کو بیچنے کے بجائے کمائی چار دن روکنا ایک پوزیشن ہے، نام کچھ بھی دیں۔
...
```
সোমবার না বেচে চার দিন আয় ধরে রাখা মানেই একটা পজিশন, নাম যাই দিন।
...
**English** — `img/en/10.png`

```
Market making on one pair, industrialised.
...
```
Market making en un par, pero industrializado.
...
```
Market making num par, mas industrializado.
...
```
صناعة سوق على زوج واحد، لكن بشكل صناعي.
...
```
بازارگردانی روی یک جفت، اما در مقیاس صنعتی.
...
```
ایک جوڑے کی مارکیٹ میکنگ، مگر بڑے پیمانے پر۔
...
```
এক পেয়ারের মার্কেট মেকিং, তবে বড় পরিসরে।
...
**English** — `img/en/11.png`

```
Three pairs whose prices imply a fourth that does not exist.
...
```
Tres pares cuyos precios implican un cuarto que no existe.
...
```
Três pares cujos preços implicam um quarto que não existe.
...
```
ثلاثة أزواج تُوحي أسعارها بسعر رابع غير موجود.
...
```
سه جفت که قیمتشان نرخ چهارمی را می‌سازد که وجود ندارد.
...
```
تین جوڑے جن کی قیمتیں ایک چوتھی شرح ظاہر کرتی ہیں جو موجود ہی نہیں۔
...
```
তিনটি পেয়ার, যাদের দাম এমন একটা চতুর্থ রেট বোঝায় যার অস্তিত্ব নেই।
...
**English** — `img/en/12.png`

```
A 1.4% spread looks like money until you price the whole loop.

Two on-chain fees and one bank charge can take it under zero on small size.
...
```
Un spread del 1,4% parece dinero hasta que cuentas el ciclo entero.

Dos comisiones on-chain y un cargo bancario lo pueden dejar bajo cero en tamaño pequeño.
...
```
Um spread de 1,4% parece dinheiro até você contar o ciclo inteiro.

Duas taxas on-chain e uma tarifa bancária podem levar isso abaixo de zero em tamanho pequeno.
...
```
فارق ١٫٤٪ يبدو ربحاً حتى تحسب الدورة كاملة.

رسوم شبكة مرتين ورسوم بنكية قد تأخذه تحت الصفر على حجم صغير.
...
```
اسپرد ۱٫۴٪ تا وقتی کل چرخه را حساب نکرده‌اید شبیه پول است.

دو کارمزد شبکه و یک کارمزد بانکی می‌تواند آن را در حجم کوچک زیر صفر ببرد.
...
```
۱٫۴٪ کا سپریڈ اُس وقت تک منافع لگتا ہے جب تک آپ پورا چکر نہ گنیں۔

دو آن چین فیس اور ایک بینک چارج چھوٹے حجم پر اسے صفر سے نیچے لے جا سکتے ہیں۔
...
```
১.৪% স্প্রেড ততক্ষণ লাভ মনে হয় যতক্ষণ পুরো চক্র হিসাব না করেন।

দুটো অন-চেইন ফি আর একটা ব্যাংক চার্জ ছোট সাইজে একে শূন্যের নিচে নিতে পারে।
...
**English** — `img/en/13.png`

```
If a leg cannot finish inside the escrow window, it is not one trade.
...
```
Si una pata no se cierra dentro de la ventana del escrow, no es una operación.
...
```
Se uma perna não fecha dentro da janela do escrow, não é uma operação.
...
```
إذا لم تُغلق الساق داخل نافذة الضمان، فهي ليست صفقة واحدة.
...
```
اگر یک پا داخل پنجره امانی بسته نشود، این یک معامله نیست.
...
```
اگر کوئی ٹانگ ایسکرو کی مدت میں مکمل نہ ہو تو یہ ایک ٹریڈ نہیں۔
...
```
কোনো পা যদি এসক্রো উইন্ডোর ভেতরে শেষ না হয়, তবে সেটা এক ট্রেড নয়।
...
**English** — `img/en/14.png`

```
The best quote for 200 KGST tells you nothing about where 200,000 KGST actually fills.
...
```
La mejor cotización para 200 KGST no te dice nada sobre dónde se ejecutan 200.000 KGST.
...
```
A melhor cotação para 200 KGST não diz nada sobre onde 200.000 KGST realmente executam.
...
```
أفضل سعر لـ٢٠٠ KGST لا يخبرك شيئاً عن مكان تنفيذ ٢٠٠٬٠٠٠ KGST.
...
```
بهترین قیمت برای ۲۰۰ KGST هیچ چیزی درباره جای پرشدن ۲۰۰٬۰۰۰ KGST نمی‌گوید.
...
```
۲۰۰ KGST کی بہترین قیمت یہ نہیں بتاتی کہ ۲۰۰٬۰۰۰ KGST کہاں بھرے گا۔
...
```
২০০ KGST-র সেরা দর আপনাকে বলে না ২,০০,০০০ KGST আসলে কোথায় ফিল হবে।
...
**English** — `img/en/15.png`

```
An opportunity you have to fund first is usually gone by the time the funding lands.
...
```
Una oportunidad que primero tienes que fondear suele desaparecer antes de que llegue el dinero.
...
```
Uma oportunidade que você precisa financiar primeiro geralmente some antes de o dinheiro chegar.
...
```
الفرصة التي تحتاج لتمويلها أولاً تختفي عادةً قبل وصول التمويل.
...
```
فرصتی که اول باید تأمین مالی‌اش کنید معمولاً پیش از رسیدن پول از بین می‌رود.
...
```
جس موقع کے لیے پہلے پیسے بھیجنے پڑیں، وہ عموماً پیسے پہنچنے سے پہلے ختم ہو جاتا ہے۔
...
```
যে সুযোগের জন্য আগে টাকা পাঠাতে হয়, টাকা পৌঁছানোর আগেই সেটা সাধারণত চলে যায়।
...
**English** — `img/en/16.png`

```
In a one-way market only your losing side gets hit.

You buy all the way down and never sell.
...
```
En un mercado de una sola dirección solo te ejecutan la punta perdedora.

Compras todo el camino hacia abajo y nunca vendes.
...
```
Num mercado de mão única só a sua ponta perdedora é executada.

Você compra o caminho inteiro para baixo e nunca vende.
...
```
في سوق أحادي الاتجاه لا يُنفَّذ إلا جانبك الخاسر.

تشتري طوال طريق الهبوط ولا تبيع أبداً.
...
```
در بازار یک‌طرفه فقط سمت بازنده شما پر می‌شود.

تمام مسیر نزول می‌خرید و هیچ‌وقت نمی‌فروشید.
...
```
یک طرفہ مارکیٹ میں صرف آپ کا نقصان والا رخ بھرتا ہے۔

آپ پورے راستے خریدتے جاتے ہیں اور بیچتے کبھی نہیں۔
...
```
একমুখী বাজারে কেবল আপনার হারা দিকটাই ফিল হয়।

আপনি নামার পুরো পথ ধরে কিনতে থাকেন, বিক্রি করেন না।
...
**English** — `img/en/17.png`

```
Escrow ties capital up while an order is live.

A book full of your quotes can look busy while most of your money earns nothing.
...
```
El escrow inmoviliza capital mientras la orden está viva.

Un libro lleno de tus órdenes puede parecer activo mientras la mayor parte de tu dinero no gana nada.
...
```
O escrow trava capital enquanto a ordem está viva.

Um livro cheio das suas ordens pode parecer movimentado enquanto a maior parte do seu dinheiro não rende nada.
...
```
الضمان يجمّد رأس المال ما دام الأمر قائماً.

دفتر مليء بأوامرك قد يبدو نشطاً بينما معظم مالك لا يربح شيئاً.
...
```
تا وقتی سفارش زنده است، امانی سرمایه را قفل نگه می‌دارد.

دفتری پر از سفارش‌های شما می‌تواند شلوغ به نظر برسد در حالی که بیشتر پولتان هیچ درآمدی ندارد.
...
```
آرڈر زندہ رہنے تک ایسکرو سرمایہ روکے رکھتا ہے۔

آپ کے آرڈرز سے بھری بُک مصروف لگ سکتی ہے جبکہ آپ کا زیادہ تر پیسہ کچھ نہیں کما رہا۔
...
```
অর্ডার জীবিত থাকা পর্যন্ত এসক্রো পুঁজি আটকে রাখে।

আপনার দরে ভরা বুক ব্যস্ত দেখাতে পারে, অথচ আপনার বেশির ভাগ টাকা কিছুই কামাচ্ছে না।
...
**English** — `img/en/18.png`

```
Compute-coin books are shallow.

The rate you multiplied by is often not the rate you get — and the position you are left holding is the hardest one to exit.
...
```
Los libros de compute coins son poco profundos.
...
```
Os livros de compute coins são rasos.
...
```
دفترهای کامپیوت‌کوین کم‌عمق‌اند.
...
```
کمپیوٹ کوائن کی بُکس کم گہری ہوتی ہیں۔
...
**English** — `img/en/19.png`

```
Pick the network at the ticket, not at the pair.
...
```
Elige la red en el ticket, no en el par.
...
```
Escolha a rede no ticket, não no par.
...
```
اختر الشبكة عند التذكرة، لا عند الزوج.
...
```
شبکه را در تیکت انتخاب کنید، نه در جفت معاملاتی.
...
```
نیٹ ورک جوڑے پر نہیں، ٹکٹ پر منتخب کریں۔
...
```
নেটওয়ার্ক বাছুন টিকিটে, পেয়ারে নয়।
...
**English** — `img/en/20.png`

```
Deepest liquidity of any quote on the book.
...
```
La liquidez más profunda de todo el libro.
...
```
A liquidez mais profunda do livro inteiro.
...
```
پوری بُک کی سب سے گہری لیکویڈیٹی۔
...
```
পুরো বুকের সবচেয়ে গভীর লিকুইডিটি।
...
**English** — `img/en/21.png`

```
Already listed on Binance, so its price is not ours to invent.
...
```
Ya cotiza en Binance, así que el precio no lo inventamos nosotros.
...
```
Já listado na Binance, então o preço não é invenção nossa.
...
```
مُدرَج على Binance، فالسعر ليس من اختراعنا.
...
```
از قبل در بایننس لیست شده، پس قیمتش را ما نمی‌سازیم.
...
```
پہلے سے بائنانس پر لسٹڈ، اس لیے قیمت ہماری ایجاد نہیں۔
...
```
আগে থেকেই Binance-এ লিস্টেড, তাই দামটা আমাদের বানানো নয়।
...
**English** — `img/en/22.png`

```
Bank transfer, card-to-card, mobile wallet, cash in person.
...
```
Transferencia bancaria, tarjeta a tarjeta, billetera móvil, efectivo en persona.
...
```
Transferência bancária, cartão para cartão, carteira móvel, dinheiro na mão.
...
```
حوالة بنكية، بطاقة إلى بطاقة، محفظة موبايل، نقداً باليد.
...
```
حواله بانکی، کارت به کارت، کیف پول موبایل، نقدی حضوری.
...
```
بینک ٹرانسفر، کارڈ ٹو کارڈ، موبائل والٹ، بالمشافہ نقد۔
...
```
ব্যাংক ট্রান্সফার, কার্ড টু কার্ড, মোবাইল ওয়ালেট, সামনাসামনি নগদ।
...
**English** — `img/en/23.png`

```
Native ETH straight out of MetaMask — the UI builds the transaction, you sign it.
...
```
ETH nativo directo desde MetaMask: la interfaz arma la transacción, tú la firmas.
...
```
ETH nativo direto do MetaMask: a interface monta a transação, você assina.
...
```
‏ETH أصلي مباشرة من MetaMask: الواجهة تبني المعاملة وأنت توقّعها.
...
```
‏ETH بومی مستقیم از متامسک: رابط تراکنش را می‌سازد، شما امضا می‌کنید.
...
```
‏MetaMask سے براہِ راست نیٹو ETH: انٹرفیس ٹرانزیکشن بناتا ہے، آپ دستخط کرتے ہیں۔
...
```
MetaMask থেকে সরাসরি নেটিভ ETH: ইন্টারফেস ট্রানজেকশন বানায়, আপনি সই করেন।
...
**English** — `img/en/24.png`

```
No minimum payout. No listing queue. No SWIFT.
...
```
Sin pago mínimo. Sin cola de listado. Sin SWIFT.
...
```
Sem pagamento mínimo. Sem fila de listagem. Sem SWIFT.
...
```
لا حد أدنى للسحب. لا طابور إدراج. لا SWIFT.
...
```
بدون حداقل برداشت. بدون صف لیست شدن. بدون SWIFT.
...
```
کوئی کم از کم حد نہیں۔ کوئی لسٹنگ قطار نہیں۔ کوئی SWIFT نہیں۔
...
```
কোনো ন্যূনতম পেআউট নেই। লিস্টিং লাইন নেই। SWIFT নেই।
...
**English** — `img/en/25.png`

```
Run the jobs — your node settles AI compute work in KGST or a regional compute coin.

List the payout on the P2P book at your own price.
...
```
Procesa los trabajos: tu nodo liquida el cómputo de IA en KGST o en un compute coin regional.

Publica el cobro en el libro P2P a tu precio.
...
```
Rode as tarefas: seu nó liquida o trabalho de IA em KGST ou num compute coin regional.

Publique o pagamento no livro P2P ao seu preço.
...
```
نفّذ المهام: عقدتك تسوّي عمل الذكاء الاصطناعي بـKGST أو بعملة حوسبة إقليمية.

اعرض العائد على دفتر P2P بسعرك أنت.
...
```
کارها را اجرا کنید: نود شما کار هوش مصنوعی را با KGST یا یک کامپیوت‌کوین منطقه‌ای تسویه می‌کند.

درآمد را با قیمت خودتان در دفتر P2P بگذارید.
...
```
کام چلائیں: آپ کا نوڈ اے آئی کمپیوٹ کو KGST یا علاقائی کمپیوٹ کوائن میں طے کرتا ہے۔

کمائی اپنی قیمت پر P2P بُک میں رکھیں۔
...
```
কাজ চালান: আপনার নোড এআই কম্পিউট কাজ KGST বা আঞ্চলিক কম্পিউট কয়েনে সেটল করে।

নিজের দামে P2P বুকে আয়টা তুলুন।
...
**English** — `img/en/26.png`

```
Zero.

No threshold to reach, no queue to join, no wire to explain to a compliance desk.
...
Sin umbral que alcanzar, sin cola a la que sumarse, sin transferencia que explicarle a un área de cumplimiento.
...
Sem limite para atingir, sem fila para entrar, sem transferência para explicar a um time de compliance.
...
لا حد تبلغه، لا طابور تنضم إليه، لا حوالة تشرحها لقسم الامتثال.
...
نه آستانه‌ای برای رسیدن، نه صفی برای پیوستن، نه حواله‌ای که به واحد انطباق توضیح دهید.
...
نہ کوئی حد چھونی ہے، نہ کسی قطار میں لگنا ہے، نہ کسی کمپلائنس ڈیسک کو ٹرانسفر سمجھانی ہے۔
...
ছোঁয়ার মতো কোনো সীমা নেই, দাঁড়ানোর মতো লাইন নেই, কমপ্লায়েন্স ডেস্ককে বোঝানোর মতো ট্রান্সফার নেই।
...
**English** — `img/en/27.png`

```
KGST is the rail. aiGEL, aiETB and aiBTN are the regional compute coins.
...
```
KGST es el riel. aiGEL, aiETB y aiBTN son los compute coins regionales.
...
```
KGST é o trilho. aiGEL, aiETB e aiBTN são os compute coins regionais.
...
```
‏KGST هو السكة. وaiGEL وaiETB وaiBTN هي عملات الحوسبة الإقليمية.
...
```
‏KGST ریل اصلی است. aiGEL و aiETB و aiBTN کامپیوت‌کوین‌های منطقه‌ای‌اند.
...
```
‏KGST بنیادی ریل ہے۔ aiGEL، aiETB اور aiBTN علاقائی کمپیوٹ کوائنز ہیں۔
...
```
KGST হলো মূল রেল। aiGEL, aiETB আর aiBTN হলো আঞ্চলিক কম্পিউট কয়েন।
...
**English** — `img/en/28.png`

```
Every taker who wants out right now pays someone for the privilege.
...
```
Todo el que quiere salir ahora mismo le paga a alguien por el privilegio.
...
```
Todo mundo que quer sair agora paga alguém pelo privilégio.
...
```
كل من يريد الخروج الآن يدفع لأحدهم مقابل ذلك.
...
```
هرکس همین حالا بخواهد خارج شود، بابتش به کسی پول می‌دهد.
...
```
جسے ابھی نکلنا ہے وہ اس سہولت کی قیمت کسی کو ادا کرتا ہے۔
...
```
এখনই বেরোতে চাওয়া প্রত্যেকে সেই সুবিধার দাম কাউকে দেয়।
...
**English** — `img/en/29.png`

```
Thin pairs pay the widest edge and fill slowly.

KGST-USDT pays the least and fills constantly.
...
```
Los pares delgados pagan el margen más ancho y se ejecutan lento.

KGST-USDT paga lo mínimo y se ejecuta sin parar.
...
```
Pares rasos pagam a margem mais larga e executam devagar.

KGST-USDT paga o mínimo e executa o tempo todo.
...
```
الأزواج الضحلة تدفع أوسع هامش وتُنفَّذ ببطء.

‏KGST-USDT يدفع الأقل ويُنفَّذ باستمرار.
...
```
جفت‌های کم‌عمق بیشترین حاشیه را می‌دهند و کند پر می‌شوند.

‏KGST-USDT کمترین را می‌دهد و مدام پر می‌شود.
...
```
پتلے جوڑے سب سے چوڑا مارجن دیتے ہیں مگر آہستہ بھرتے ہیں۔

‏KGST-USDT سب سے کم دیتا ہے مگر مسلسل بھرتا ہے۔
...
```
পাতলা পেয়ার সবচেয়ে চওড়া মার্জিন দেয়, ফিল হয় ধীরে।

KGST-USDT দেয় সবচেয়ে কম, ফিল হয় অবিরাম।
...
**English** — `img/en/30.png`

```
The base side locks the moment an order is selected, and stays locked for 15 minutes.
...
```
El lado base se bloquea en cuanto se selecciona la orden, y queda bloqueado 15 minutos.
...
```
O lado base trava no momento em que a ordem é selecionada, e fica travado por 15 minutos.
...
```
الجانب الأساسي يُقفل لحظة اختيار الأمر، ويبقى مقفلاً ١٥ دقيقة.
...
```
سمت پایه همان لحظه انتخاب سفارش قفل می‌شود و ۱۵ دقیقه قفل می‌ماند.
...
```
آرڈر منتخب ہوتے ہی بنیادی رخ لاک ہو جاتا ہے اور ۱۵ منٹ لاک رہتا ہے۔
...
```
অর্ডার নির্বাচিত হওয়ার মুহূর্তেই বেস দিক লক হয়, আর ১৫ মিনিট লক থাকে।
